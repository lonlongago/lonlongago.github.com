		...
		<div @click.stop="" @touchstart.stop="" class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}" transition="calendar" transition-mode="out-in">
			<div class="calendar-tools" v-if="type != 'time'">
			    ...
			    <div class="text center">
			        <input type="number" v-model="year"  @change="render(year,month)" min="1970" max="2100" maxlength="4" number="" />/ {{month+1}}
			    </div>
			</div>
			<table cellpadding="5" v-if="type != 'time'">
			    ...
			    <tbody>
				    <tr v-for="(day,k1) in days">
					    <td v-for="(child,k2) in day" :class="{'today':child.today,'disabled':child.disabled}" @click="select(k1,k2,$event)" @touchstart="select(k1,k2,$event)"> 
					    	{{child.day}}
					    </td>
				    </tr>
			    </tbody>
			...
			<div class="calendar-time" v-show="type=='datetime24'||type=='datetime'||type=='time'">
			    <div class="timer">
			        <input type="number" v-model="hour"  min="0" max="23" maxlength="2" number="" />
			        <span>时</span>
			        <input v-show="type != 'datetime24'" type="number" v-model="minute"  min="0" max="59" maxlength="2" number="" />
			        <span v-show="type != 'datetime24'">分</span>
			        <input v-show="type != 'datetime24'" type="number" v-model="second"  min="0" max="59" maxlength="2" number="" />
...
//	                    that.value=that.year+that.sep+that.zero(that.month+1)+that.sep+that.zero(that.days[k1][k2].day);
	                    let _newDays = 0;
	                	_newDays = that.year+that.sep+that.zero(that.month+1)+that.sep+that.zero(that.days[k1][k2].day);
	                    that.$emit('update:value', _newDays);
	                    that.$emit('update:type','data');
	                    that.show=false;
