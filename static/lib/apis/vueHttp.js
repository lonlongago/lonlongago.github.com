...
SZXJ.getLocalTimeP =  function(nS) {
  var time = new Date(nS);
  return time.Format("yyyy-MM-dd hh:mm:ss");
  // return new Date(parseInt(nS) * 1000).toLocaleString().substr(0,17);
}
SZXJ.getLocalTimePHMS =  function(nS) {
...
SZXJ.getLocalTimeHMS =  function(nS) {
  var time = new Date(nS * 1000);
  return time.Format("hh:mm:ss");
  // return new Date(parseInt(nS) * 1000).toLocaleString().substr(0,17);
}
SZXJ.getLocalTimeFir =  function(nS) {
  var time = new Date(nS);
...
SZXJ.http = (This,_method, _url, _data, successfn, errorfn) => {
  // 设置登入请求头
//if (localStorage != undefined && localStorage.getItem('JSESSIONID')) {
...
//}
  ...
  if (_method === 'get') {
	...
	console.log(_URL + str);
  	Vue.http.get(_URL + str, null).then(
      (response) => {
