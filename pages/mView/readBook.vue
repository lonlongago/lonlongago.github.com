	...
	<div style="font-size: 12px;">
      ...
      <transition name="fade">
        <div class="read_bottom" v-if = "readBottom">
          <div class="directory" @click="showMenu()">
            <a href="javascript:;" >
              <img src="http://cdn.qcacg.com/mobile/img/app_read/menu.png"/>
              <p>目录</p>
            </a>
          </div>
          <div class="night" @click="showReset()">
            <a href="javascript:;" >
              <img src="http://cdn.qcacg.com/mobile/img/app_read/size.png"/>
              <p>设置</p>
            </a>
          </div>
          <div class="font" @click="updataModel()">
            <a href="javascript:;" v-if="model" key="night">
              <img src="http://cdn.qcacg.com/mobile/img/app_read/Moon.png"/>
              <p>夜间</p>
            </a>
            <a href="javascript:;" v-else key="daytime">
              <img src="http://cdn.qcacg.com/mobile/img/app_read/sunlight.png"/>
              <p>日间</p>
            </a>
          </div>
          <div class="comment" @click="showComment()">
            <a href="javascript:;" >
              <img src="http://cdn.qcacg.com/mobile/img/app_read/com.png"/>
              <p>评论</p>
            </a>
          </div>
        </div>
        ...
        <div class="size" v-if="setBol"> 
            <div class="typeface">
              <div class="setFontSize">
                <span style="vertical-align: top;"><img src="http://cdn.qcacg.com/mobile/img/app_read/36573c0fa183d35.png" @click="reduce()" /></span>
                <div class="sizeRange">
                   ...
                   <mu-slider v-model="value1" :step="2" class="demo-slider" @change="sizeChange()" :min="16" :max="36" />
                </div>
                <span style="vertical-align: top;"><img src="http://cdn.qcacg.com/mobile/img/app_read/b6d116ed94491e05e4177ccfd02b1d07.png" @click="increase()" /></span>
              </div>
      ...
      <transition name="fade">
        <div class="header_ReadBook" v-show="readNav">
          <div style="height: 0.171rem; background-color: #fff; width: 100%;"></div>	
      ...
      <!--侧边目录部分-->
        ...
        <div class="read_menu" v-show="bookMenu" >
          ...
          <div class="menu_content" id="readMenu">
          	<div style="height: 0.171rem;position: fixed;z-index: 88;background: white;width: 100%;"></div>
  ...
  asyncData:function(context){
    ...
    if(context.req.headers && context.req.headers.cookie){
      ...
      if(SZXJ.getCookie(context.req.headers,'tamps')){
        ...
        obj.value1 = parseFloat(obj.obj.fontSize.split('rem')[0]);
      }
      //检查cookie是否有model
...
//			self.ws.opener().reload();  //webview重新加载
    ...
    sizeChange:function(){
      this.obj.fontSize = this.value1 + "px";
      this.$nextTick(function(){
        this.pages = Math.ceil(this.$refs.content.scrollWidth/(this.$refs.content.clientWidth+16));
        this.page = 1;
      });
    ...
    nextPage:function(){
      if(this.page < this.pages){
          ...
          if(window.plus){
    			plus.nativeUI.toast( "没有下一章节了");
    		}
//        this.$refs.appAlert.setMessage("没有下一章节了");
        }
      }
    },
    //上下翻页点击进入下一页
  ...
  //      var offsetHeight  
    ...
    //展示导航栏和设置栏
    openMenu:function(){
      console.log('readNav:'+this.readNav);
      if(!this.readNav){
        this.readNav = true;
...
//      this.showMenu1 = true;
    ...
    prePage:function(){
    	if(this.page <= 1){
          ...
          this.wid_pic = 0;
          if(window.plus){
    			plus.nativeUI.toast( "没有上一章了");
    		}
//        this.$refs.appAlert.setMessage("没有上一章了");
        }
    	}else{
    		this.page = this.page - 1;
    		this.wid_pic =0 - this.offsetWidth*(this.page -1);
...
//	            	location.href = "/mView/login";
    ...
    reduce:function() {
      ...
      this.obj.fontSize = this.value1.toString()+"px";
      this.$nextTick(function(){
        this.pages = Math.ceil(this.$refs.content.scrollWidth/(this.$refs.content.clientWidth+16));
        this.page = 1;
      })
    ...
    showComment:function(){
        if(window.plus){
	    	this.webBook = plus.webview.create(appPathList.netWorks+'/commentFirst/'+this.bookId,'commentFirst',
	    	{
			    'popGesture':'close',
			    'titleNView':{'backgroundColor':'#ffffff','titleText':'评论列表',titleOverflow:'ellipsis','titleColor':'#000000','autoBackButton':true,splitLine:{color:'#C3C3C3'}},
			    'statusbar':{'background':'#ffffff'},
			    "backButtonAutoControl":'close'
            });
            this.webBook.onloading= function(){
...
//	    	location.href = "/mView/commentFirst/"+this.bookId;
	 	}
    },
    //关闭导航栏和设置栏
    closeNav:function(e){
    ...
    getMenuFn:function(){
      ...
      SZXJ.http(this,'get',appPathList.queryBookDirectory+"?bookId="+this.bookId,{},(res)=>{
        ...
        for(var i = 0; i<this.volumeCustomList.length;i++){
          this.allLength = this.volumeCustomList[i].contentEntityList.length;
        }
...
<style>
  ...
  .size .typeface .fontFamily{
    ...
    background-image: url(http://cdn.qcacg.com/mobile/static/img/app_read/up.png);
    background-repeat: no-repeat;
  ...
  .theFamily .f_one .span_yes{
    ...
    width: 0.2048rem;
    height: 0.2048rem;
    background-image: url(http://cdn.qcacg.com/mobile/static/img/app_read/right.png);
    background-repeat: no-repeat;
  ...
  .section .preContent{
    ...
    display: inline-block;
    float: left;
    background-image: url(http://cdn.qcacg.com/mobile/img/app_read/left.png);
    background-repeat: no-repeat;
  ...
  .section .nextContent{
    ...
    display: inline-block;
    float: right;
    background-image: url(http://cdn.qcacg.com/mobile/img/app_read/right__.png);
    background-repeat: no-repeat;
