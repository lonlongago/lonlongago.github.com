<template>
	<div style="background-color: #f1f1f1;">
		<script src="/lib/jquery.min.js" ></script>
		<script src="/lib/cropper.min.js"></script>
		<div id="worksDestails">
			<alert ref="alert"></alert>
			<myheader></myheader>
			<div class="painter_Header"></div>
			<div class="worksDestails">
				<div class="painter_Nav_C">
					<div class="painter_Nav">
						<img class="imgLogo" src="data:image/png;base64," />
						<ul>
							<li class="active">
								<a href="/painter/spaceIndex"><span>首页</span></a>
							</li>
							<li>
								<a href="/painter/works"><span>画作</span></a>
							</li>
							<li>
								<a href="/painter/painter"><span>画师</span></a>
							</li>
							<li>
								<a href="/painter/draft"><span>约稿</span></a>
							</li>
							<li>
								<a href="#"><span>活动</span></a>
							</li>
						</ul>
						<div class="inputBlock relative">
							<input v-model="search" class="" type="text" placeholder="请输入关键词..." />
							<img @click="searchFn" src="data:image/png;base64," />
							<!--<img src="" />-->
						</div>
					</div>
				</div>
				<div class="worksDestails_content">
					<div class="content_header">
						<div class="content_block">
							<div class="head">
								<a :href="'/painter/space/' + painter.userId"><img :src=" path.ImagePath + 'Controller' + painter.userHead " onerror="javascript:this.src='data:image/jpeg;base64,';" src="data:image/jpeg;base64," alt="" /></a>
							</div>
							<ul class="info">
								<li class="frist_Capable">
									<span class="name"><a style="color:white;" :href="'/painter/space/' + userId">{{painter.userName}}</a></span>
									<div @click="setAttention(painter.userId,1)" class="follow" v-if=" isFans == 0 || !loginFlag ">+关注</div>
									<div @click="setAttention(painter.userId,0)" class="follow concern" v-else>已关注</div>
									<img @click="mackFn" v-if=" isSolicitContributions == 1 " class="email" src="/img/painter/emil.png" />
								</li>
								<li class="introduce">个人说明：{{painter.information}}</li>
							</ul>
							<ul class="attribute">
								<li class="border">{{picture.styleName}}</li>
								<li class="border">{{picture.className}}</li>
								<li class="border">共{{album.pictureAmount}}p</li>
								<li class="border border_none">{{szxj.getLocalTimeP(album.createTime)}}</li>
							</ul>
						</div>
					</div>
					<div class="content_Middle">
						<div class="title">{{picture.pictureName}}</div>
						<div class="drawings" style="overflow: hidden;">
							<img :src="picture.picturePath" alt="" />
							<p v-if="btnFlag == 1" v-for="(obj,index) in barrageList" class="Barrage" :style="'left:'+ obj.axisX + 'px; top:' + obj.axisY +'px;'">
								<span></span>{{obj.message}}<b @click="barrageIndexFn(index)"></b>
							</p>
							<div class="block_in" @click="onmousedown" @contextmenu.prevent="onmousedown" :style=" BarrageFlag ? 'display:block;':'display:none;' "></div>
							<div class="edit_in">
								<div class="btnAn C" @click="AppearFn"></div>
								<div v-if=" btnFlag == 1 " @click="seeBtnFn(2)" class="btnAn A" style="margin-top: 10px;"></div>
								<div v-if=" btnFlag == 2 " @click="seeBtnFn(1)" class="btnAn B" style="margin-top: 10px;"></div>
							</div>
						</div>
						<div class="lamented" :style="'bottom:' + bottom + 'px;'">
							<div class="lamented_block">
								<input placeholder="(～￣▽￣)～ (～￣▽￣)～吐槽吐槽来一发~~~ " type="text" v-model="words" />
								<div class="Prompt" style="">可以输入30个字</div>
								<div @click="SetPublishBarrageFn" class="btn publish_button">发表吐槽</div>
								<div @click="noneBarrageFn" class="btn publish_cancel">不吐槽了</div>
							</div>
						</div>
						<!--<div class="prompt-text hidden" id="promptText" :style=" 'opacity:' +  opacity  + ';'">
							<div class="prompt-cover">
								<span id="promptMain">蓝色区域才可以吐槽哦~</span>
							</div>
						</div>-->
						<div class="text">
							{{picture.pictureDescription}}
						</div>
					</div>

					<div class="content_Bottom">
						<ul class="share">
							<li>分享：</li>
							<li><img src="" alt="" /></li>
							<li><img src="" alt="" /></li>
							<li><img src="" alt="" /></li>
						</ul>
						<ul class="browse">
							<li><img src="/img/painter/eyes.png" alt="" /></li>
							<li class="border">浏览({{album.hitAmount}})</li>
							<li><img src="/img/painter/love_03.png" alt="" /></li>
							<li @click="laudAmountFn" style="cursor: pointer;">喜欢({{album.laudAmount}})</li>
						</ul>
					</div>
					<div class="album">
						<ul class="list">
							<li v-for="(obj,index) in pictureList" @click="pictureIndexFn(obj,index)" :class=" index == pictureIndex ? 'border':'' ">
								<img :src=" obj.picturePath + '&imageView2/1/w/106/h/116/interlace/1/q/100|imageslim' " alt="" />
							</li>
						</ul>
					</div>
				</div>
				<div style="min-width: 1200px;">
					<div class="lower">
						<div class="leftSection">
							<div class="caption">
								<span @click="sectionFn(1)" class="discuss active" :class="sectionFlag == 1 ? 'active' : ''">评论</span>
								<span @click="sectionFn(2)" class="reward" :class="sectionFlag == 2 ? 'active' : ''">打赏</span>
								<div class="hr" :style="sectionFlag == 1 ? 'left:5px;' : 'left:95px;'"></div>
							</div>

							<!--切换-->
							<div class="comment" v-if=" sectionFlag == 1 ">
								<div class="report">
									<div class="headimg">
										<img v-if="loginFlag" :src=" path.ImagePath + 'Controller' + status.userImage " onerror="javascript:this.src='data:image/jpeg;base64,';" src="data:image/jpeg;base64," alt="" />
										<img v-else src="/img/sss10.png" alt="" />
									</div>
									<textarea class="entry" name="" rows="" cols="" v-model="commentContent"></textarea>
									<div @click="addCommentFn" class="publish">发表评论</div>
								</div>
								<div class="criticism">
									<ul class="list">
										<li class="speaker" v-for=" obj in commentList">
											<div class="commentFloor">
												<div class="head">
													<a :href="'/painter/space/' + obj.userId"><img :src=" path.ImagePath + 'Controller' +  obj.userHead" onerror="javascript:this.src='data:image/jpeg;base64,';" src="data:image/jpeg;base64," alt="" /></a>
												</div>
												<ul class="attribute">
													<li class="name">
														<a :href="'/painter/space/' + obj.userId">{{obj.userName}}</a>
													</li>
													<li class="substance">{{obj.commentContent}}</li>
													<li class="time">
														<span>{{szxj.getLocalDate(obj.commentDate)}}</span>
														<span>{{szxj.getLocalTimePHMS(obj.commentDate)}}</span>
														<span @click="replyFn(obj.commentId,0,obj.userId)" class="reply">回复</span></li>
												</ul>
											</div>
											<div class="replyFloor" v-for=" (reply,index) in obj.replyEntityList " v-if=" index < 5 ">
												<div class="reply_head">
													<a :href="'/painter/space/' + reply.userId"><img :src=" path.ImagePath + 'Controller' +  reply.userHead " onerror="javascript:this.src='data:image/jpeg;base64,';" src="data:image/jpeg;base64," alt="" /></a>
												</div>
												<ul class="reply_attribute">
													<li class="reply_substance">
														<span class="reply_name"><a :href="'/painter/space/' + reply.userId">{{reply.userName}}</a></span>
														<span>{{reply.replyContent}}</span>
													</li>
													<li class="reply_time">
														<span>{{szxj.getLocalDate(reply.replyDate)}}</span>
														<span>{{szxj.getLocalTimePHMS(reply.replyDate)}}</span>
														<span @click="replyFn(obj.commentId,1,obj.userId)" class="reply">回复</span></li>
												</ul>
											</div>
											<div class="addReply" :style=" obj.commentId == commentId && addReplyFlag ? 'display:block' : 'display:none' ">
												<div class="headimg">
													<img v-if="loginFlag" :src=" path.ImagePath + 'Controller' + status.userImage " onerror="javascript:this.src='data:image/jpeg;base64,';" src="data:image/jpeg;base64," alt="" />
													<img v-else src="/img/sss10.png" alt="" />
												</div>
												<textarea class="entry" name="" rows="" cols="" v-model="replyContent"></textarea>
												<div @click="addReplyFn" class="publish">发表回复</div>
											</div>
											<ul class="pageReply" v-if="obj.page.length > 1">
												<li class="flip" @click="setReplyFn(obj.index, 1)">上一页</li>
												<li v-for="(page,index2) in obj.page" @click="setReplyFn(obj.index, 3, index2)" :class="obj.pageNum == (index2 + 1) ? 'active number':'number' " 
												v-if="((index2 + 1)<= 5 && obj.pageNum< 3 || ((index2 + 1) > obj.pageNum - 3 &&  index2< obj.pageNum)) || ( ((index2 + 1)<= obj.pageNum + 2 &&　(index2 + 1) > obj.pageNum) || (obj.pageNum > obj.pageCount - 3 && (index2 + 6) > obj.pageCount) )">{{page}}</li>
												<li class="flip" @click="setReplyFn(obj.index, 2)">下一页</li>
											</ul>
										</li>
									</ul>
									<page-component ref="page"
									   			:pageNum="commentObj.pageNum"
									   			@update:pageNum="val => commentObj.pageNum = val"
									   			:pageSize="commentObj.pageSize"
									   			@update:pageSize="val => commentObj.pageSize = val"
									   			></page-component>	
								</div>
							</div>

							<!--切换-->
							<div class="premium" v-if=" sectionFlag == 2 ">
								<ul class="gift">
									<li @click="setGiftNumFn(100,'甜品')" :class=" giftNum == 100 ? 'active':''  ">
										<img class="cake" src="/img/painter/gift_01.png" alt="" />
										<div class="quantum">100张好人卡</div>
									</li>
									<li @click="setGiftNumFn(233,'狗粮')" :class=" giftNum == 233 ? 'active':''  ">
										<img class="dog" src="/img/painter/gift_02.png" alt="" />
										<div class="quantum">233张好人卡</div>
									</li>
									<li @click="setGiftNumFn(520,'玫瑰')" :class=" giftNum == 520 ? 'active':''  ">
										<img src="/img/painter/gift_03.png" alt="" />
										<div class="quantum">520张好人卡</div>
									</li>
									<li class="margin" @click="setGiftNumFn(666,'小熊')" :class=" giftNum == 666 ? 'active':''  ">
										<img src="/img/painter/gift_04.png" alt="" />
										<div class="quantum">666张好人卡</div>
									</li>
									<li @click="setGiftNumFn(888,'蛋糕')" :class=" giftNum == 888 ? 'active':''  ">
										<img class="bigcake" src="/img/painter/gift_05.png" alt="" />
										<div class="quantum">888张好人卡</div>
									</li>
									<li @click="setGiftNumFn(1314,'王冠')" :class=" giftNum == 1314 ? 'active':''  ">
										<img src="/img/painter/gift_06.png" alt="" />
										<div class="quantum thousand">1314张好人卡</div>
									</li>
									<li @click="setGiftNumFn(3000,'法拉利')" :class=" giftNum == 3000 ? 'active':''  ">
										<img src="/img/painter/gift_07.png" alt="" />
										<div class="quantum thousand">3000张好人卡</div>
									</li>
									<li class="margin" @click="setGiftNumFn(5000,'火箭')" :class=" giftNum == 5000 ? 'active':''  ">
										<img src="/img/painter/gift_08.png" alt="" />
										<div class="quantum thousand">5000张好人卡</div>
									</li>
								</ul>
								<div class="explain">赠送一只{{giftName}}，共计<b>{{giftNum}}</b>张好人卡</div>
								<div class="balance explain">好人卡余额：{{cardamount}}</div>
								<div @click="giveGoodFn()" class="give">立即赠送</div>
							</div>
						</div>
						<div class="rightSection">
							<div class="other">
								<div class="head">
									<span class="title">其他画集</span>
									<span class="more"><a :href="'/painter/space/' + painter.userId">更多</a></span>
								</div>
								<ul class="album">
									<li v-for="obj in painterAlbumList">
										<a :href=" + obj.albumId"><img :src=" obj.coverPicturePath + '&imageView2/1/w/120/h/132/interlace/1/q/100|imageslim'" alt="" /></a>
									</li>
								</ul>
							</div>
							<div class="activity">
								<div class="head">相关推荐</div>
								<ul class="recommend">
									<li onclick="location.href = 'http://localhost:3000/newCollect'">
										<img src="/img/Illustration/11_01.jpg" alt="" />
									</li>
									<li>
										<img src="/img/Illustration/11_02.jpg" alt="" />
									</li>
									<li onclick="location.href = 'http://localhost:3000/catalog/71'">
										<img src="/img/Illustration/11_03.jpg" alt="" />
									</li>
								</ul>
							</div>
						</div>
					</div>
				</div>
			</div>

			<!--邀请函-->

			<div v-if="mackFlag" class="Invitation">
				<div class="background_black"></div>
				<div class="Invitation_mack">
					<div class="Invitation_block">
						<img @click="mackFn" class="shut" src="/img/painter/shut.png" alt="" />
						<ul class="Invitation_ul">
							<li class="whatever">
								<div class="title">企划方</div>
								<div class="name">{{status.userName}}</div>
							</li>
							<li class="whatever">
								<div class="title">画师</div>
								<div class="name">{{painter.userName}}</div>
							</li>
							<li class="whatever">
								<div class="title">标题</div>
								<input class="caption" type="text" placeholder="请简单描述您的需求" v-model="obj.requirePaint" />
							</li>
							<li class="whatever">
								<div class="title top">需求补充</div>
								<textarea class="demand" type="text" placeholder="补充您的需求" v-model="obj.demand"></textarea>
							</li>
							<li class="whatever">
								<div class="title top margin">类型</div>
								<ul class="choose">
									<li v-if="className == '' " class="addLabel">添加标签</li>
									<li v-else class="addLabel selected">{{className}}</li>
									<li class="recommendedlabels"><span>——</span>&nbsp;&nbsp;<span>推荐标签</span>&nbsp;&nbsp;<span>——</span></li>
									<li class="listLabel">
										<div v-for="obj in classTypeList" class="label" @click="setClassWorksFn(obj.typeName,obj.pId)">{{obj.typeName}}</div>
									</li>
								</ul>
							</li>
							<li class="whatever">
								<div class="title top margin">画风</div>
								<ul class="choose">
									<li v-if="styleName == '' " class="addLabel">添加标签</li>
									<li v-else class="addLabel selected">{{styleName}}</li>
									<li class="recommendedlabels"><span>——</span>&nbsp;&nbsp;<span>推荐标签</span>&nbsp;&nbsp;<span>——</span></li>
									<li class="listLabel">
										<div v-for="obj in styleTypeList" class="label" @click="setStyleWorksFn(obj.typeName,obj.pId)">{{obj.typeName}}</div>
									</li>
								</ul>
							</li>
							<li class="whatever">
								<div class="title">订单金额</div>
								<!--<input type='text' onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)" onblur="this.v();" />  只能输入数字-->  
								<input class="caption short" type="text" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-99999999-]+/,'');}).call(this)" onblur="this.v();" v-model="amountFir" />
								<div class="transition">~</div>
								<input class="caption short" type="text" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-99999999-]+/,'');}).call(this)" onblur="this.v();" v-model="amountSec" />
							</li>
							<li class="whatever">
								<div class="title">联系方式(QQ)</div>
								<input class="caption short" type="text" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-99999999-]+/,'');}).call(this)" onblur="this.v();" oninput="if(value.length&gt;11)value=value.slice(0,11)" min="111111" v-model="obj.qq" maxlength="11"  />
							</li>
							<li class="whatever">
								<div class="title">交稿时间</div>
								<input class="caption short" type="text" @click="showDate" index="0" readonly="readonly" v-model="obj.deliveryTime" placeholder="例：2017-1-1" />
								<date1 :type="Datetype" :show="showA" :value="obj.deliveryTime"
								        	@update:Datetype="val => Datetype = val"
								        	@update:show="val => showA = val"
								        	@update:value="val => obj.deliveryTime = val"
								        	 :x="x" :y="y" :begin="begin" :end="end" :range="range"></date1> 
								<!--<date-time :type.sync="Datetype" :show.sync="showA" :value.sync="obj.deliveryTime" :x="x" :y="y" :begin="begin" :end="end" :range="range"></date-time>-->
							</li>
							<li class="whatever">
								<div class="title top">案例</div>
								<img class="exhibition" v-if="displayImg" :src=" displayImg " />
								<div class="upload" @click="clickFileActiveFn">点击上传</div>
							</li>
						</ul>
						<div class="bottom">
							<div class="true" @click="submitMackFn">确认</div>
							<div class="true cancel" @click="mackFn">取消</div>
						</div>
						<div class="bottom" style="display: none;">
							<div class="true">同意</div>
							<div class="true cancel">取消</div>
						</div>
					</div>
				</div>
			</div>
			<!--上传图片-->
			<input type="file" ref="file" style="display: none;" v-on:change="fileUpload" />
		</div>
		<div style="height: 100px;"></div>
		<myfooter></myfooter>

	</div>
</template>

<script>
	import Header from '~/components/Header'
	import PathList from '~/components/conf'
	import appPathList from '~/components/conf-app'
	import SZXJ from '~/components/vueHttp'
	import Alert from '~/components/Alert'
	import axios from 'axios'
	import Footer from '~/components/Footer'
	import pageComponent from '~/components/pageComponent'
	import dateTime from '~/components/date'
	// this.$refs.alert.setMessage(false,'message',function(){})
	export default {
		components: {
			'alert': Alert,
			'myheader': Header,
			'myfooter': Footer,
			'pageComponent':pageComponent,
			'date1': dateTime
		},
		asyncData: function(context) {
			var obj={}; 
		    obj.id=context.params.id;
			return obj;
		},
		data: function() {
			return {
				path: PathList,
				szxj: SZXJ,
				albumId: '',
				mackFlag: false,
				amountFir: '',
				amountSec: '',
				picture: [],
				pictureList: [],
				album: [],
				painter: [],
				painterAlbumList: [],
				classTypeList: [],
				styleTypeList: [],
				className: '',
				styleName: '',
				loginFlag: false,
				imgFlag: false,
				updataImg: '',
				imgUrl: '',
				updataFlag: 0,
				displayImg: '',
				displayImgTow: '',
				displayImgThr: '',
				status: [],
				userId: '',
				pictureIndex: 0,
				commentContent: '',
				commentList: [],
				addReplyFlag: false,
				sectionFlag: 1,
				commentId: '',
				giftNum: 100,
				giftName: '甜品',
				pages: '',
				pageNum: '',
				numberPage: '',
				pageNumber: [],
				isFans: '',
				replyUserId: '',
				replyStatus: '',
				replyContent: '',
				replyNumber: [],
				styleType: '',
				classType: '',
				hasLaud: '',
				cardamount: '',
				isSolicitContributions: '',
				bottom: -185,
				timer: '',
				timerSec: '',
				opacityTimer: '',
				BarrageFlag: false,
				btnFlag: 1,
				timeFlag: false,
				opacityFlag: false,
				words: '',
				opacity: 0,
				barrageList: [],
				barragepId: '',
				barrageObj: {
					moudle: 1,
					moudleId: '',
					pageSize: 100,
				},
				obj: {
					painterUserId: '',
					requirePaint: '',
					demand: '',
					paintOrderStyleIds: '',
					paintOrderTypeIds: '',
					budgetMoneyExt: '',
					qq: '',
					deliveryTime: '',
					imageCaseList: [],
				},
				painterObj: {
					userId: '',
					pageNum: 1,
					pageSize: 4,
					sort: 0,
				},
				commentObj: {
					pageNum: 1,
					pageSize: 10,
					sort: 1,
					albumId: '',
				},
				// 时间控件
				showA: false,
				showB: false,
				Datetype: 'date', // date datetime datetime24
				begin: '',
				end: '',
				x: 0,
				y: 0,
				range: false,
				// 时间控件结束
				search:''
			}
		},
		mounted: function() { // ready -->
			this.$nextTick(function() {
				this.albumId = this.id;
				this.getValueFn();
				this.setCommentFn();
				var self = this;
				console.log(this.$refs);
				this.$refs.page.setFn(function(){
			    	this.$emit('update:pageNum', this.pageNum);
			    	this.$emit('update:pageSize', this.pageSize);
		    		self.setCommentFn();
	    		});
			})
		},
		methods: {
		    setMessage(flag,message,Fn) {
			    this.$refs.alert.setMessage(flag,message,Fn);
			},
		    searchFn:function(){
		      	location.href = encodeURI('/painter/works/' + this.search) ;
		    },
			clickFileActiveFn: function() { // 模拟点击效果
				const ie = navigator.appName === 'Microsoft Internet Explorer' ? 'true' : 'false';
				const file = this.$refs.file;
				if(ie === 'true') {
					file.click();
				} else {
					const a = document.createEvent('MouseEvents'); // FF的处理
					a.initEvent('click', true, true);
					file.dispatchEvent(a);
				}
			},
			fileUpload(e) { // 文件上传
				var This = this;
				var file = e.target.files[0];
				var formData = new FormData();
				formData.append("file", file);
				$.ajax({
					type: 'POST',
					processData: false, // 告诉jQuery不要去处理发送的数据
					contentType: false, // 告诉jQuery不要去设置Content-Type请求头
					url: PathList.caseFileUpload,
					data: formData,
					beforeSend: function(request) {
						request.setRequestHeader("JSESSIONID", localStorage.getItem('JSESSIONID'));
					},
					success: function(data) {
						This.imgUrl = data.data.filePath;
						This.displayImg = PathList.ImagePath + This.imgUrl;
						This.obj.imageCaseList.push(This.imgUrl);
					},
					error: function(data) {}
				});
			},
			setGiftNumFn: function(a, b) {
				this.giftNum = a;
				this.giftName = b;
			},
			setClassWorksFn: function(a, b) {
				this.className = a;
				this.classType = b;
			},
			sectionFn: function(a) {
				this.sectionFlag = a;
			},
			replyFn: function(a, b, c) {
				if(this.addReplyFlag) {
					this.addReplyFlag = false;
				} else {
					this.addReplyFlag = true;
					this.commentId = a;
					this.replyStatus = b;
					this.replyUserId = c;
				}
			},
			setStyleWorksFn: function(a, b) {
				this.styleName = a;
				this.styleType = b;
			},
			setAttention: function(a, b) {
				SZXJ.http(this, 'post', PathList.saveOrCancelAttention, {
					receiveId: a
				}, (response) => {
					this.isFans = b;
				},(err)=>{
					this.$refs.alert.setMessage(true, err.body.msg,function(){
		      			location.href='/login';
		      		});
				});
			},
			pictureIndexFn: function(a, b) {
				this.picture = a;
				this.pictureIndex = b;
				this.barrageObj.moudleId = this.picture.pictureId;
				this.SetBarrage();
			},
			mackFn: function() {
				if(this.loginFlag && this.mackFlag) {
					this.mackFlag = false;
				} else if(this.loginFlag && !this.mackFlag && this.userId !== this.status.userId) {
					this.mackFlag = true;
				}
			},
			giveGoodFn: function() {
				var data = {
					albumId: this.albumId,
					cardAmount: this.giftNum
				}
				SZXJ.http(this, 'get', PathList.painterGiveCard, data, (response) => {
//					var Utils = this.$refs.vueAlert ? this.$refs.vueAlert : this.$parent.$refs.vueAlert;
					this.$refs.alert.setMessage(false,'打赏成功',function(){})
//					Utils.setMessage(false, '打赏成功');
					this.AppearFn();
					return;
				},(err)=>{
					this.$refs.alert.setMessage(true,err.body.msg,function(){})
				});
			},
			submitMackFn: function() {
				this.mackFlag = false;
		        this.obj.mtime = new Date();
		        this.obj.myear=this.obj.mtime.getFullYear();
		        this.obj.mmon=this.obj.mtime.getMonth()+1;
		        this.obj.mdat=this.obj.mtime.getDate();
		        this.obj.year=this.obj.deliveryTime.split("-")[0];
		        this.obj.mon=this.obj.deliveryTime.split("-")[1];
		        this.obj.dat=this.obj.deliveryTime.split("-")[2];
				this.obj.painterUserId = this.userId;
				this.obj.paintOrderStyleIds = this.styleType;
				this.obj.paintOrderTypeIds = this.classType;
				this.obj.budgetMoneyExt = this.amountFir + '-' + this.amountSec;
				this.obj.budgetMoneyAc=Number(this.amountFir);
				this.obj.budgetMoneyEd=Number(this.amountSec);
				if(this.obj.requirePaint == ''){
//					var Utils = this.$refs.vueAlert ? this.$refs.vueAlert : this.$parent.$refs.vueAlert;
					this.$refs.alert.setMessage(true,'亲，请输入标题',function(){});
//					Utils.setMessage(true, '请正确填写内容');
					return;
				}else if(this.obj.demand == ''){
					this.$refs.alert.setMessage(true,'亲，请完善您的需求',function(){});
					return;
				}else if(this.obj.paintOrderTypeIds == ''){
					this.$refs.alert.setMessage(true,'亲，请选择类型',function(){});
					return;
				}else if(this.obj.paintOrderStyleIds == ''){
					this.$refs.alert.setMessage(true,'亲，请选择画风',function(){});
					return;
				}else if(this.obj.budgetMoneyExt == '' || this.obj.budgetMoneyAc == '' || this.obj.budgetMoneyEd == '' || this.obj.budgetMoneyAc <= 0 || this.obj.budgetMoneyEd <= 0 || this.obj.budgetMoneyEd <= this.obj.budgetMoneyAc){
					this.$refs.alert.setMessage(true,'亲，订单金额输入有误哦',function(){});
					return;
				}else if(this.obj.qq == ''|| this.obj.qq<=0 || this.obj.qq.length<6){
					this.$refs.alert.setMessage(true,'亲，QQ号输入有误',function(){});
					return;
				}else if(this.obj.deliveryTime == ''){
					this.$refs.alert.setMessage(true,'亲，时间不能为空',function(){});
					return;
				}else if(this.obj.year < this.obj.myear){
					this.$refs.alert.setMessage(true,'亲，年份恒久远，何必强求',function(){});
					return;
				}else if(this.obj.year == this.obj.myear && this.obj.mon < this.obj.mmon){
					this.$refs.alert.setMessage(true,'亲，岁月如流水，时间不可倒流哦',function(){});
					return;
				}else if(this.obj.year == this.obj.myear && this.obj.mon == this.obj.mmon && this.obj.dat < this.obj.mdat){
					this.$refs.alert.setMessage(true,'亲，有多少日子能够重来～～',function(){});
					return;
				}else if(this.obj.year == this.obj.myear && this.obj.mon == this.obj.mmon && this.obj.dat == this.obj.mdat){
					this.$refs.alert.setMessage(true,'亲，为了给您展示更好的画作，请留给作家至少一天的时间去完成哟～～o～～',function(){});
					return;
				}else if(this.obj.imageCaseList == ''){
					this.$refs.alert.setMessage(true,'亲，图片有误',function(){});
					return;
				}else {
					SZXJ.http(this, 'post', PathList.solicitContribution, this.obj, (response) => {
						var Utils = this.$refs.vueAlert ? this.$refs.vueAlert : this.$parent.$refs.vueAlert;
						this.$refs.alert.setMessage(false,'申请成功',function(){})
//						Utils.setMessage(false, '申请成功');
						return;
					},(err)=>{
						if(err) {
				      		this.$refs.alert.setMessage(true,'亲,输入有误，请仔细检查一遍',function(){})
				      	}
					});
				}
			},
			showDate(e) {
				e.stopPropagation();
				const that = this;
				that.showA = false;
				that.showB = false;
				if(e.target.getAttribute('index') === '0') {
					that.showA = true;
				} else if(e.target.getAttribute('index') === '1') {
					that.showB = true;
				}
				that.x = e.target.offsetLeft;
				that.y = e.target.offsetTop + e.target.offsetHeight + 8;
				const bindHide = (_event) => {
					_event.stopPropagation();
					if(e.target.getAttribute('index') === '0') {
						that.showA = false;
					} else if(e.target.getAttribute('index') === '1') {
						that.showB = false;
					}
					document.removeEventListener('click', bindHide, false);
				};
				setTimeout(() => {
					document.addEventListener('click', bindHide, false);
				}, 500);
				return false;
			},
			getValueFn: function() {
				var self = this;
				var data = {
					albumId: this.albumId,
				};
				SZXJ.http(this, 'get', PathList.albumDetail, data, (response) => {
					this.album = response.data.data.album;
					this.painter = response.data.data.painter;
					this.userId = response.data.data.painter.userId;
					this.pictureList = response.data.data.pictureList;
					this.isFans = response.data.data.isFans;
					this.hasLaud = response.data.data.hasLaud;
					this.pictureIndexFn(this.pictureList[0]);
					this.getPainterAlbum(this.userId);
					this.SetContribution(this.userId);
				});

				SZXJ.http(this, 'get', PathList.typeList, {
					typeType: 1
				}, (response) => {
					this.classTypeList = response.data.data
				});
				SZXJ.http(this, 'get', PathList.typeList, {
					typeType: 2
				}, (response) => {
					this.styleTypeList = response.data.data
				});			    
				SZXJ.http(this, 'get', PathList.getStatus, {}, (response) => {
					this.status = response.data.status;
					this.loginFlag = response.data.loginFlag;
					if(response.data.loginFlag) {
						this.SetcardamountFn();
					}
				});
			},
			SetContribution: function(a) {
				SZXJ.http(this, 'get', PathList.queryCanContribution, {
					userId: a
				}, (response) => {
					this.isSolicitContributions = response.data.isSolicitContributions;
				});
			},
			SetBarrage: function() {
				SZXJ.http(this, 'get', PathList.setBarrage, this.barrageObj, (response) => {
					this.barrageList = response.data.data;
//					if(response.code == 401){
//			                this.$refs.alert.setMessage(false,'操作过于频繁',function(){})
//			                this.amountYueFn();
//			                return;
//			        }    
				});
			},
			SetcardamountFn: function() {
				SZXJ.http(this, 'get', PathList.cardamount, {}, (response) => {
					this.cardamount = response.data.amount.payCardAmount + response.data.amount.welfareCardAmount;
				});
			},
			laudAmountFn: function() {
				SZXJ.http(this, 'get', PathList.giveLaud, {
					albumId: this.albumId
				}, (response) => {
					this.album.laudAmount += 1;
					this.hasLaud= true;
				},(err)=>{
					this.$refs.alert.setMessage(true, err.body.msg,function(){
		      			
		      		});
				});
			},
			setCommentFn: function(a) {
				this.commentObj.albumId = this.albumId;
				var self = this;
				
				SZXJ.http(this, 'get', PathList.painterListComment, this.commentObj, (response) => {
					this.pages = response.data.data.pages;
					this.pageNum = response.data.data.pageNum;
					this.commentObj.pageNum = response.data.data.pageNum;
					for (var i = 0; i < response.data.data.list.length; i++) {
			        	response.data.data.list[i].pageNum = 1;
			        	response.data.data.list[i].index = i;
			        }
					this.commentList = response.data.data.list;
					this.$refs.page.setPageCount(response.data.data.pages);
					this.setReplyPagesNumberFn();
					
				});
			},
			setReplyPagesNumberFn: function() { // 计算回复的分页
				for(var i = 0; i < this.commentList.length; i++) {
					if(!this.commentList[i].replyEntityList) {
						this.commentList[i].page = [];
						continue;
					}
					this.commentList[i].page = [];
					// 计算评论楼中楼回复的总页数
					if(this.commentList[i].totalCount / 5 <= 1) {
						this.commentList[i].pageCount = 1;
					} else {
						if(this.commentList[i].totalCount % 5 === 0) {
							this.commentList[i].pageCount = this.commentList[i].totalCount / 5;
						} else {
							this.commentList[i].pageCount = parseInt((this.commentList[i].totalCount / 5), 10) + 1;
						}
					}
					for(var j = 1; j <= this.commentList[i].pageCount; j++) {
						this.commentList[i].page.push(j);
					}
				}
			},
			setReplyFn: function(index, flag, index2) { // 回复的回复
				
				var obj = this.commentList;
				this.commentList = [];
				if(flag == 1) {
					if(obj[index].pageNum != 1) {
						obj[index].pageNum = obj[index].pageNum - 1;
					} else {
						obj[index].pageNum = 1;
					}
				}
				if(flag == 2) {
					if(obj[index].pageNum >= obj[index].replyEntityList.page[obj[index].replyEntityList.page.length - 1]) {
						obj[index].pageNum = obj[index].replyEntityList.page[obj[index].replyEntityList.page.length - 1];
					} else {
						obj[index].pageNum = obj[index].pageNum + 1;
					}
				}
				if(flag == 3) {
					obj[index].pageNum = (index2 + 1);
				}

				var data = {
					pageNum: obj[index].pageNum,
					pageSize: 5,
					commentId: obj[index].commentId
				}
				SZXJ.http(this, 'get', PathList.painterListReply, data, (response) => {
					response.data.pageNum = obj[index].pageNum;
					response.data.page = obj[index].page;
					obj[index].replyEntityList = response.data.data;
					this.commentList= obj;
					this.setReplyPagesNumberFn();
				});
			},
			addCommentFn: function() {
				var data = {
					commentType: 1,
					commentContent: this.commentContent,
					albumId: this.albumId,
				}
				SZXJ.http(this, 'post', PathList.addComment, data, (response) => {
					this.commentContent= '';
					this.setCommentFn();
					this.addReplyFlag = false;
				},(err)=>{
					this.$refs.alert.setMessage(true, err.body.msg,function(){});
				});
			},
			addReplyFn: function() {
				var data = {
					commentId: this.commentId,
					replyContent: this.replyContent,
					replyUserId: this.replyUserId,
					replyStatus: this.replyStatus
				}
				SZXJ.http(this, 'post', PathList.saveReply, data, (response) => {
					this.replyContent= '';
					this.setCommentFn();
					this.addReplyFlag = false;
				},(err)=>{
					this.$refs.alert.setMessage(true,err.data.msg,function(){})
				});
			},
			getPainterAlbum: function(a) {
				this.userId = a;
				this.painterObj.userId = a;
				SZXJ.http(this, 'get', PathList.painterAlbums, this.painterObj, (response) => {
					this.painterAlbumList = response.data.data.list;
				});
			},
			AppearFn: function() {
				if(this.loginFlag) {
					clearInterval(this.timerSec);
					var This = this;
					this.timeFlag = true;
					this.opacityFlag = false;
					this.timer = setInterval(function() {
						if(This.timeFlag) {
							var ispeed = Math.floor(-This.bottom / 3);
							This.bottom = ispeed + This.bottom;
							if(This.bottom == 0) {
								clearInterval(This.timer);
								This.$set('timeFlag', false);
							};
						}
					}, 30);
				} else {
					this.$refs.alert.setMessage(true, '尚未登录',function(){
		      			location.href='/login';
		      	});
					return;
				}
			},
			seeBtnFn: function(a) {
				this.btnFlag = a;
			},
			barrageIndexFn: function(a) {
				this.barrageList.splice(a, 1);
			},
			noneBarrageFn: function() {
				clearInterval(this.timer);
				this.timeFlag= false;
				this.BarrageFlag = false;
				this.words= '';
				var This = this;
				this.timerSec = setInterval(function() {
					This.bottom -= 6;
					if(This.bottom < -185) {
						clearInterval(This.timerSec);
					};
				}, 30);
			},
			SetPublishBarrageFn: function() {
				if(SZXJ.trimToStr(this.words) == '') {
					return;
				}
				this.BarrageFlag = true;
				var This = this;
				this.opacityTimer = setInterval(function() {
					if(This.opacityFlag) {
						This.opacity = This.opacity + 0.1;
						if(This.opacity >= 1) {
							clearInterval(This.opacityTimer);
							This.opacityFlag = false;
							This.opacityTimerFn(1);
						};
					} else {
						clearInterval(This.opacityTimer);
					}
				}, 40);
			},
			opacityTimerFn: function(a) {
				clearInterval(this.opacityTimer);
				this.opacity = a;
				var This = this;
				var delaytime = setTimeout(function() {
					var opacityTimerSec = setInterval(function() {
						This.opacity = This.opacity - 0.1;
						if(This.opacity <= 0) {
							This.opacityFlag = false;
							clearInterval(opacityTimerSec);
							clearTimeout(delaytime);
						};
					}, 40);
				}, 2000);
			},
			onmousedown: function(event) {
				var e = event || window.event;
				if(e.button == 0) {
					var data = {
						axisX: e.offsetX,
						axisY: e.offsetY,
						message: this.words,
						moudleId: this.barrageObj.moudleId,
						moudle: 1,
					}
					SZXJ.http(this, 'get', PathList.saveBarrage, data, (response) => {
						this.BarrageFlag = false;
						this.words= '';
						this.SetBarrage();
						this.noneBarrageFn();
					});
				} else if(e.button === 2 || e.which === 2 || !e.which && e.button === 4 || (e.target || e.srcElement).alt) {
					this.BarrageFlag = false;
					this.noneBarrageFn();
					this.words='';
				}
			}
		},
		head: function() {
			return {
				title: '画师作品页-QC轻小说',
				meta: [
					{
						charset: 'UTF-8'
					},
					{
						name: 'viewport',
						content: 'width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=0'
					},
					{
						name: 'format-detection',
						content: 'telephone=no'
					},
					{
						name: 'apple-touch-fullscreen',
						content: 'yes'
					},
					{
						name: 'apple-mobile-web-app-capable',
						content: 'yes'
					},
					{
						name: 'apple-mobile-web-app-status-bar-style',
						content: 'default'
					},
					{
						name: 'keywords',
						content: 'QC,QCACG,QC轻小说,轻小说,轻文,原创轻小说,轻小说投稿'
					},
					{
						name: 'description',
						content: 'QC原创轻小说，一个在二次元盛起的魔法圣殿，拥有魔幻与神奇色彩，为二次元提供轻小说投稿与画师约稿相结合的平台。加入我们，接受契约，用轻松奇幻的文字留下你心中的故事。 '
					},
					{
						name: 'baidu_union_verify',
						content: '1dbb2d68c26ba734e502097619803d73'
					}
				],
				link: [{
						rel: 'stylesheet',
						href: '/css/Date.css'
					},
					{
						rel: 'stylesheet',
						href: '/css/headerOrFooter.css'
					},
					{
						rel: 'stylesheet',
						href: '/painter/css/main_worksDestails.css'
					},
					{
						rel: 'stylesheet',
						href: '/css/cropper.min.css'
					},
					{
						rel: 'stylesheet',
						href: '/css/font-awesome.min.css'
					}
				]
			}
		}
	}
</script>

<style>
	.block_in {
		cursor: url('/img/painter/view_cupsor.cur'), default;
	}
	input::-webkit-outer-spin-button,  
	input::-webkit-inner-spin-button{  
	    -webkit-appearance: none !important;  
	    margin: 0;  
	}  
	#worksDestails .worksDestails .lower .leftSection .criticism .list .speaker .pageReply li.active {
		background-color: #00a1d6;
	    color: #fff;
	    transition: all .35s ease;
	    -webkit-transition: all .35s ease;
	}
</style>