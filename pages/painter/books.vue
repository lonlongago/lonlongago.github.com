		...
		<div id="space_Index">
			...
			<div id="books">
				<div class="book_con">
					...
					<ul class="books_S">
						<li v-for="books in BookList">
							<div class="space_books_pic">
								<a style="display: block;width: 100%;height: 100%;" :href="'/catalog/' + books.bookId">
								<img v-bind:src="path.ImageBookPath + books.bookCoverImage" alt="" onerror="javascript:this.src='/img/z-imgBj.jpg';" />
								</a>
							</div>
							<div class="space_books_con">
								<p class="title">{{books.bookName}}</p>
								<div class="content">{{books.bookIntroduction}}</div>
							</div>
						</li>
					</ul>
				</div>
...
//		  	this.$set('total', SZXJ.getQueryString('page') ? SZXJ.getQueryString('page') : '1');  //此段报错
	...
	methods: {
		getValueFn:function(){ // 默认请求
			    ...
			    var self = this;
			    this.$refs.page2.setFn(function(){
...
//						Utils.setMessage(false, '申请成功');
	    ...
	    getValueFn:function(){
	      ...
	      this.RequestObj.userId = this.id;
	      SZXJ.http(this,'get',PathList.queryBookList,{ userId:this.id }, (response) => {
//		  SZXJ.http(this,'get',PathList.queryBookList, this.RequestObj, (response) => {
			this.whole = response.data;
		  	this.$refs.page.setPageCount(response.data.bookListData.pages);
	        this.BookList = response.data.bookListData.list;
